...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'operators' AND policyname = 'DEV: anon full access to operators'
  ) THEN
    EXECUTE 'CREATE POLICY "DEV: anon full access to operators" ON public.operators FOR ALL TO anon USING (true) WITH CHECK (true)';
  END IF;
END
$do$;
