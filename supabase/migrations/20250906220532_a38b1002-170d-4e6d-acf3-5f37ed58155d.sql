-- First, let's check and fix the storage policies for admin access
-- Create storage policies for operator-media bucket to allow admin uploads
...
USING (
  ...
  EXISTS (
    ...
    AND (
      storage.foldername(name)[1] = operators.id::text OR
      operators.logo_url LIKE '%' || name || '%' OR 
      operators.hero_image_url LIKE '%' || name || '%'
    )
  )
);