...
-- 2. Remove duplicate cron jobs (keep only one)
SELECT cron.unschedule('invoke-schedule-publisher-every-minute');
SELECT cron.unschedule('content-scheduler');
...
-- 6. Add policy for content_sections to ensure proper access
CREATE POLICY IF NOT EXISTS "Admin: manage content_sections"
